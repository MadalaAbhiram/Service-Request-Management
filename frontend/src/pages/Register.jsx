...
function CustomCursor() {
  ...
  return (<>
    <div ref={dotRef} style={{
      position: "fixed", width: 8, height: 8, background: "#00d4ff", borderRadius: "50%",
...
export default function Register({ onBack }) {
  ...
  const handleSubmit = async (e) => {
    e.stopPropagation();
    setErr("");
    if (!name || !email || !phone || !pass) { setErr("Please fill all fields."); return; }
    setLoading(true);
    try {
      const response = await registerApi({ name, email, phone, password: pass, role: "USER" });
      ...
      doneTimerRef.current = setTimeout(() => navigate("/login"), 1500);
    } catch (error) {
      setErr(error.message || "Unable to reach registration service.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div onClick={handleBgClick}
      style={{
        position: "relative", width: "100vw", height: "100vh", overflow: "hidden", cursor: "none",
      ...
      {/* brand */}
      <div style={{
        position: "absolute", top: 28, left: 48, zIndex: 10,
            ...
            {!done ? (<>
              <div style={{ marginBottom: 28, textAlign: "center" }}>
                ...
                <div style={{ fontSize: 13, color: "rgba(255,255,255,.4)", letterSpacing: ".1em" }}>
                  Register to manage service requests with your team.
                </div>
              </div>
              ...
              <div>
                ...
                <div style={{position:'absolute',left:'-9999px',top:'auto',width:1,height:1,overflow:'hidden'}} aria-hidden="true">
                  ...
                  <input type="password" name="new-password" autoComplete="new-password" defaultValue="" />
                </div>
                <label style={{
                  display: "block", fontSize: 10, letterSpacing: ".25em",
                  color: "rgba(0,212,255,.6)", textTransform: "uppercase", marginBottom: 6
                }}>
                  Full Name
                </label>
                <input className="water-input" name="name" autoComplete="name" type="text" placeholder="Your name"
                ...
                <input className="water-input" name="tel" autoComplete="tel" type="tel" placeholder="+91 98765 43210"
                ...
                <input className="water-input" name="new-password" autoComplete="new-password" type="password" placeholder="••••••••"
                  value={pass} onChange={e => setPass(e.target.value)}
                  onKeyDown={e => e.key === "Enter" && handleSubmit(e)} />

                {err && (
                  <div style={{
                    fontSize: 12, color: "rgba(255,80,80,.8)", marginBottom: 12,
                ...
                <button className="login-btn" disabled={loading} onClick={handleSubmit}>
                  {loading ? (<><span className="spinner" /><span>Registering...</span></>) : "Register →"}
                </button>
                ...
                <div style={{
                  ...
                  Already registered?{" "}
                  <span style={{
                    color: "rgba(0,212,255,.7)", cursor: "pointer",
              ...
              <div style={{ textAlign: "center", padding: "20px 0" }}>
                ...
                <div style={{
                  fontFamily: "'Orbitron',monospace", fontSize: "clamp(14px,3vw,20px)",
                ...
                <div style={{ fontSize: 13, color: "rgba(255,255,255,.5)", letterSpacing: ".08em" }}>
                  You can now log in and manage service requests.
                </div>
...
// (placed after the component function for clarity; effect must be inside component)
